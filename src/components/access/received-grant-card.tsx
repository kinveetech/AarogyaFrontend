...
export interface ReceivedGrantCardProps {
  grant: AccessGrant
}

export function ReceivedGrantCard({ grant }: ReceivedGrantCardProps) {
  const { label, badgeVariant, daysRemaining, status } = getGrantStatus(
    grant.expiresAt,
    grant.revoked,
  )
  const isInactive = status === 'expired' || status === 'revoked'
  const patientLabel = grant.patientSub
  const initials = getInitials(patientLabel)
  ...
  return (
    <Box
      ...
      data-testid="received-grant-card"
    >
      <Flex align="center" gap="4" flexWrap={{ base: 'wrap', md: 'nowrap' }}>
        ...
        <Box flex="1" minW="0">
          <Text
            ...
            color="text.primary"
            lineHeight="1.3"
            data-testid="received-grant-patient"
          >
        ...
        {/* Status badge only — doctors cannot revoke grants given to them */}
        <Flex
          align="center"
          gap="3"
          flexShrink={0}
